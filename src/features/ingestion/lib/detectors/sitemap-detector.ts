...
export class SitemapDetector {
   ...
   */
  async detect(baseUrl: string): Promise<string[]> {
    // 1. 사이트맵 URL 찾기 (robots.txt 포함)
    const sitemapUrl = await this.findSitemapUrl(baseUrl);
    if (!sitemapUrl) return [];

    // 2. 사이트맵 콘텐츠 가져오기
    const xmlData = await this.fetchSitemapContent(sitemapUrl);
    if (!xmlData) return [];

    // 3. 파싱하여 URL 목록 반환
    return this.parseUrls(xmlData);
  }
   ...
   * @private
   */
  private async findSitemapUrl(baseUrl: string): Promise<string | null> {
   ...
   * @private
   */
  private async fetchSitemapContent(url: string): Promise<string | null> {
    try {
   ...
   * @private
   */
  private parseUrls(xmlData: string): string[] {
